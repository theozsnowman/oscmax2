...
// This will cause it to look for 'conditions_content.php'
  ...
  $_GET['filename'] = FILENAME_DEFINE_CONDITIONS_CONTENT;
  ...
  switch ($_GET['action']) {
    case 'save':
      if ( ($_GET['lngdir']) && ($_GET['filename']) ) {
        ...
        tep_redirect(tep_href_link(FILENAME_DEFINE_CONDITIONS, 'lngdir=' . $_GET['lngdir']));
      }
      break;
  }
...
<!-- body_text //-->
    <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr>
        <td><?php echo tep_draw_form('lng', FILENAME_DEFINE_CONDITIONS, '', 'get'); ?>
...
?>
          <tr>
            <td><?php echo tep_draw_form('language', FILENAME_DEFINE_CONDITIONS, 'lngdir=' . $_GET['lngdir'] . '&amp;filename=' . $_GET['filename'] . '&amp;action=save'); ?>
              <table border="0" cellspacing="0" cellpadding="2">
                ...
                <tr>
                  <td align="right"><?php if ($file_writeable) { echo tep_image_submit('button_save.gif', IMAGE_SAVE) . '&nbsp;<a href="' . tep_href_link(FILENAME_DEFINE_CONDITIONS, 'lngdir=' . $_GET['lngdir']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'; } else { echo '<a href="' . tep_href_link(FILENAME_DEFINE_CONDITIONS, 'lngdir=' . $_GET['lngdir']) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a>'; } ?></td>
                </tr>
              </table>
            </form></td>
          </tr>
...
?>
          ...
          <tr>
            <td><?php echo '<a href="' . tep_href_link(FILENAME_DEFINE_CONDITIONS, 'lngdir=' . $_GET['lngdir']) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a>'; ?></td>
          </tr>
<?php
    }
  } else {
...
?>
          <tr>
            <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="smallText"><a href="<?php echo tep_href_link(FILENAME_DEFINE_CONDITIONS, 'lngdir=' . $_GET['lngdir'] . '&amp;filename=' . $filename); ?>"><b><?php echo $filename; ?></b></a></td>
<?php
    ...
    if ($dir) {
      ...
      while ($file = $dir->read()) {
        if (substr($file, strrpos($file, '.')) == $file_extension) {
          echo '                <td class="smallText"><a href="' . tep_href_link(FILENAME_DEFINE_CONDITIONS, 'lngdir=' . $_GET['lngdir'] . '&amp;filename=' . $file) . '">' . $file . '</a></td>' . "\n";
          if (!$left) {
            echo '              </tr>' . "\n" .
                 '              <tr>' . "\n";
          }
