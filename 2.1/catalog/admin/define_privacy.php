...
// This will cause it to look for 'conditions_content.php'
  ...
  $_GET['filename'] = FILENAME_DEFINE_PRIVACY_CONTENT;
  ...
  switch ($_GET['action']) {
    case 'save':
      if ( ($_GET['lngdir']) && ($_GET['filename']) ) {
        ...
        tep_redirect(tep_href_link(FILENAME_DEFINE_PRIVACY, 'lngdir=' . $_GET['lngdir']));
      }
      break;
  }
...
?>
          ...
          <tr>
            <td><?php echo '<a href="' . tep_href_link(FILENAME_DEFINE_PRIVACY, 'lngdir=' . $_GET['lngdir']) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a>'; ?></td>
          </tr>
<?php
    }
  } else {
