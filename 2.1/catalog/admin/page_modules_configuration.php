  ...
  if (tep_not_null($action)) {
    switch ($action) {
      case 'insert':
        ...
        tep_db_query("insert into " . TABLE_PM_CONFIGURATION . " (pm_id, pm_title, pm_filename, pm_page, pm_active, pm_sort_order, date_added) values ('" . (int)$pm_id . "', '" . $pm_title . "', '" . $pm_filename . "', '" . tep_db_input($pm_page) . "', '" . tep_db_input($pm_active) . "', '" . tep_db_input($pm_sort_order) . "', now())");
...
<?php
  $pm_query_raw = "select pm_id, pm_title, pm_filename, pm_page, pm_active, pm_sort_order, date_added, last_modified from " . TABLE_PM_CONFIGURATION . " order by pm_page, pm_sort_order";
...
?>
                ...
                <td class="dataTableContent" align="center"><?php echo $links['pm_active']; ?></td>
              ...
              <tr>
                <td colspan="6"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    ...
                    <td class="smallText" align="right"><?php echo $pm_split->display_links($pmquery_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $_GET['page']); ?></td>
                  </tr>
<?php
  if (empty($action)) {
?>
                  <tr>
                    <td colspan="6" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_PM_CONFIGURATION, 'page=' . $_GET['page'] . '&action=new') . '">' . tep_image_button('button_insert.gif', IMAGE_NEW_TAX_RATE) . '</a>'; ?></td>
                  </tr>
<?php
  }
?>
...
<?php
  ...
  switch ($action) {
    case 'new':
      ...
      $contents[] = array('text' => '<br>' . TEXT_PM_FILENAME . '<br>' . tep_draw_input_field('pm_filename', 'FILENAME_'));
	  $contents[] = array('text' => '<br>' . TEXT_PM_PAGE . '<br>' . tep_draw_input_field('pm_page'));
	  $contents[] = array('text' => '<br>' . TEXT_PM_ACTIVE . '<br>' . tep_draw_input_field('pm_active'));
    ...
    case 'edit':
	  ...
	  $contents[] = array('text' => '<br>' . TEXT_PM_ACTIVE . '<br>' . tep_draw_input_field('pm_active', $trInfo->pm_active));
