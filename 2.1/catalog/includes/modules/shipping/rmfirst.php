...
class rmfirst {
	...
	function rmfirst() {
		...
		$this->version = ((defined('MODULE_SHIPPING_RMFIRST_VERSION')) ? MODULE_SHIPPING_RMFIRST_VERSION : '?.?');
		$this->code = 'rmfirst';
	...
	function install() {
		...
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Version', 'MODULE_SHIPPING_RMFIRST_VERSION', '2.2.1', 'Sort order of display (1 shown first 99 etc shown last to customer)', '6', '0', now())");
		...
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Maximum value to ship', 'MODULE_SHIPPING_RMFIRST_MAX_VALUE', '41', 'Enter the maximum value to ship', '6', '0', now())");
		...
		for ($i = 1; $i <= $this->num_zones; $i++) {
			...
			if ($i == 1) {
				...
				$shipping_table = '.1:1.39,.25:1.72,.5:2.24,.75:2.75,1:3.35,1.25:4.5,1.5:5.2,1.75:5.9,2:6.6,4:8.22,6:11.02,8:13.82,10:16.62';
				$shipping_handling = 0;
			}
			...
			tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('RM First Class Rates', 'MODULE_SHIPPING_RMFIRST_COST_" . $i ."', '" . $shipping_table . "', 'Enter values upto 5,2 decimal places. (12345.67) Example: .1:1,.25:1.27 - Weights less than or equal to 0.1Kg would cost �1.00, Weights less than or equal to 0.25g but more than 0.1Kg will cost �1.27. Do not enter KG or � symbols.', '6', '0', now())");
