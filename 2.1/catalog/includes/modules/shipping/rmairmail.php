...
class rmairmail {
	...
	function install() {
		...
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Maximum value to ship', 'MODULE_SHIPPING_RMAIRMAIL_MAX_VALUE', '41', 'Enter the maximum value to ship', '6', '0', now())");
		...
		for ($i = 1; $i <= $this->num_zones; $i++) {
			...
			if ($i == 1) {
				...
				$shipping_table = '.1:1.31,.12:1.42,.14:1.57,.16:1.7,.18:1.85,.2:1.98,.22:2.12,.24:2.24,.26:2.37,.28:2.51,.3:2.65,.34:2.89,.38:3.13,.42:3.37,.46:3.61,.5:3.85,.52:3.96,.56:4.18,.62:4.51,.68:4.84,.74:5.17,.8:5.5,.9:6.05,1:6.6,1.2:7.6,1.4:8.6,1.6:9.6,1.8:10.6,2:11.6';
				$shipping_handling = 0;
			...
			if ($i == 2) {
				...
				$shipping_table = '.1:1.82,.12:2.1,.14:2.38,.16:2.65,.18:2.93,.2:3.2,.22:3.46,.24:3.71,.26:3.96,.28:4.22,.3:4.47,.34:4.95,.38:5.43,.42:5.91,.46:6.39,.5:6.87,.52:7.09,.56:7.53,.62:8.19,.68:8.85,.74:9.51,.8:10.17,.9:11.27,1:12.37,1.2:14.57,1.4:16.77,1.6:18.97,1.8:21.17,2:23.37';
				$shipping_handling = 0;
			// MODULE_SHIPPING_RMAIRMAIL_COUNTRIES_$i
